...
export const candidateApi = {
  ...
  searchJobs: async (searchParams: any) => {
    console.log('🌐 candidateApi.searchJobs called with:', searchParams);
    ...
    try {
      console.log('📡 Making API call to /api/v1/jobs/search');
  ...
  // Simple search suggestions for autocomplete
  getSearchSuggestions: async (query: string, limit: number = 5) => {
    if (!query || query.length < 2) return { success: true, data: [] };
    
    const cacheKey = apiCache.generateKey('searchSuggestions', { query, limit });
    const cached = apiCache.get(cacheKey);
    if (cached) return cached;
    
    try {
      const response = await apiClient.get('/api/v1/jobs/search/suggestions', { 
  ...
  // Simplified Job Recommendations API (calls AI service directly)
  getRecommendedJobsAI: async (params?: { 
    top_k?: number;
    refresh_cache?: boolean;
  }) => {
    const cacheKey = apiCache.generateKey('jobRecommendations', params);
    ...
    try {
      const response = await apiClient.get('/api/v1/jobs/recommendations', { 
        params: {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        ...
        console.warn('Candidate profile not found for recommendations');
        return {
          success: true,
          data: [],
  ...
  // Legacy: Smart AI-powered job recommendations (deprecated - use getUnifiedJobRecommendations)
  getSmartJobRecommendations: async (params?: { 
    top_k?: number; 
    min_match_score?: number;
    refresh_cache?: boolean;
  }) => {
    // Redirect to unified endpoint
    console.warn('getSmartJobRecommendations is deprecated, use getUnifiedJobRecommendations instead');
    return await candidateApi.getUnifiedJobRecommendations({
      ...params,
      mode: 'smart'
    });
  },

  // Removed - AI service handles recommendations automatically
  ...
  // Trigger match calculation for a CV
  triggerMatchCalculation: async (cvId: string) => {
    const response = await apiClient.post(`/api/v1/cvs/${cvId}/calculate-match`);
    return response.data;
  },
