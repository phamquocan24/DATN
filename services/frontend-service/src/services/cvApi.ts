...
export const extractCVWithId = async (file: File, cvId: string): Promise<{
  success: boolean;
  message: string;
  cv_id: string;
  extracted_data: CVExtractResponse;
  ...
  const response = await fetch(`${AI_SERVICE_BASE_URL}/extract-cv-with-id`, {
    method: 'POST',
    body: formData,
  });
