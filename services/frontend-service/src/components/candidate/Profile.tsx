...
interface ProfileProps {
  ...
  onProfileUpdate?: () => void; // Callback to notify parent of profile updates
}
...
const Profile: React.FC<ProfileProps> = ({ 
    ...
    switch (sectionName) {
      ...
      case 'education':
        sectionData = {
          ...
          school_name: profileData.school_name || profileData.candidate_profile?.school_name
        };
        break;
      case 'personal':
        sectionData = {
  ...
  const handleSaveProfile = async (sectionName?: string) => {
    try {
      ...
      if (editedData.school_name !== undefined) updateData.school_name = editedData.school_name;
            break;
          case 'personal':
      ...
      if (response.success) {
        ...
        // Notify parent of profile update (for avatar refresh in header)
        if (onProfileUpdate) {
          onProfileUpdate();
        }
              ...
              {profileData.cv_education && profileData.cv_education.length > 0 ? (
                ...
                <div className="flex space-x-4">
                  ...
                  <div className="flex-1">
                      ...
                      {editingSections.education ? (
                        <div className="space-y-3">
                          ...
                          <input
                            ...
                            value={editedData.school_name || editedData.candidate_profile?.school_name || ''}
                            onChange={(e) => handleFieldChange('school_name', e.target.value)}
                            className="border border-gray-300 rounded-lg px-3 py-2 focus:border-[#007bff] focus:ring-1 focus:ring-[#007bff] outline-none w-full"
                          />
                        </div>
                      ) : (
                        <div>
                          ...
                          {(profileData.school_name || profileData.candidate_profile?.school_name) && (
                    ...
                    <p className="text-gray-500 text-sm">Educational background</p>
                  </div>
                </div>
              ) : (
