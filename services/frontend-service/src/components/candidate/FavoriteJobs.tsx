...
// Removed AI matching imports - no longer used in favorite jobs
...
export const FavoriteJobs: React.FC<FavoriteJobsProps> = ({ onJobClick }) => {
  ...
  const [filterCounts, setFilterCounts] = useState({
    ...
    salaryRange: {} as Record<string, number>
  });

  // Removed loadSelectedCV function - no longer used

  // Removed getMatchGrade function - no longer used

  // Removed calculateAIMatchScoresForFavoriteJobs function - no longer used
  ...
  const transformJobData = (apiJob: any): Job => {
    ...
    return {
      ...
      saved_at: apiJob.saved_at || apiJob.created_at || new Date().toISOString(),
      isSaved: true
    };
  };

  // Calculate filter counts from jobs data
  const calculateFilterCounts = (jobs: Job[]) => {
    const counts = {
      employmentType: {} as Record<string, number>,
      categories: {} as Record<string, number>,
      jobLevel: {} as Record<string, number>,
    ...
    jobs.forEach(job => {
      ...
      const empType = job.employment_type?.toLowerCase() || job.type?.toLowerCase() || 'full-time';
      counts.employmentType[empType] = (counts.employmentType[empType] || 0) + 1;

      // Category counts (from tags)
      job.tags.forEach(tag => {
        const category = tag.toLowerCase();
        counts.categories[category] = (counts.categories[category] || 0) + 1;
      });

      // Salary range counts (simplified)
      if (job.salary_min || job.salary_max) {
        let salaryRange = 'negotiable';
        if (job.salary_min && job.salary_min < 20000000) {
          salaryRange = 'under-20m';
        } else if (job.salary_min && job.salary_min >= 20000000 && job.salary_min < 50000000) {
          salaryRange = '20m-50m';
        } else if (job.salary_min && job.salary_min >= 50000000) {
          salaryRange = 'above-50m';
        }
        counts.salaryRange[salaryRange] = (counts.salaryRange[salaryRange] || 0) + 1;
      }
    });
  ...
  const fetchFavoriteJobs = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Extract locations from favorite jobs
        const locations = new Set<string>();
        transformedJobs.forEach((job: any) => {
          if (job.city_name) {
          ...
          if (job.location && job.location !== job.city_name) {
        ...
        calculateFilterCounts(transformedJobs);
  ...
  // Removed AI matching useEffect - no longer used
  ...
  useEffect(() => {
    ...
    console.log('🔀 Sorting by:', sortBy, 'Jobs count:', filtered.length);
    ...
    filtered = [...filtered].sort((a, b) => {
      switch (sortBy) {
        case 'newest':
          ...
          return dateB - dateA;
        case 'oldest':
          const oldDateA = new Date(a.saved_at || '').getTime();
          const oldDateB = new Date(b.saved_at || '').getTime();
          console.log('📅 Sorting oldest:', { 
            jobA: a.title, 
            dateA: a.saved_at, 
            jobB: b.title, 
            dateB: b.saved_at,
            result: oldDateA - oldDateB
          });
    ...
    console.log('✅ After sorting:', filtered.map(job => ({ title: job.title, saved_at: job.saved_at })));
  ...
  const JobCard = ({ job }: { job: Job }) => (
    ...
    >
      ...
      <div className="flex items-center flex-wrap gap-2 mb-4">
        ...
        {/* Removed AI Match Score Tags - no longer used */}
