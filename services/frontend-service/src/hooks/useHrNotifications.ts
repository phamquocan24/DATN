...
interface UseHrNotificationsReturn {
  unreadCount: number;
  isLoading: boolean;
  error: string | null;
...
export const useHrNotifications = (): UseHrNotificationsReturn => {
  ...
  const fetchUnreadCount = useCallback(async () => {
    try {
      ...
      const response = await hrApi.getUnreadNotificationCount();
      ...
      try {
        const notificationsResponse = await hrApi.getNotifications({ 
          limit: 100 
        });
