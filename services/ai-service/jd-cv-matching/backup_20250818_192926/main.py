...
async def calculate_match(request: MatchRequest):
    ...
    try:
        ...
        if not parsed_result:
            ...
            cursor.execute(
                """
                SELECT cv_title, cv_file_name, candidate_id FROM candidate_cvs
                WHERE cv_id = %s
                """,
                (request.cv_id,),
            )
            ...
            cv_title, file_name, cv_candidate_id = cv_info
            
            # Create basic parsed content structure
            parsed_content = {
                "full_name": (cv_title or "").replace("'s CV", "").replace(" CV", "").strip() or "Unknown",
                "email": "",
                "phone": "",
                "address": "",
