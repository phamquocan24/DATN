...
// Helper function to calculate match scores for CV with available jobs using batch endpoint
async function calculateMatchScoresForCV(cvId) {
  try {
    logger.info('Starting batch match score calculation for CV:', { cv_id: cvId });

    // Use the new batch endpoint from AI service
    const AI_MATCHING_SERVICE_URL = process.env.AI_MATCHING_SERVICE_URL || 'http://localhost:8001';
    
    const batchResponse = await axios.post(`${AI_MATCHING_SERVICE_URL}/api/v1/ai/batch-calculate-matches`, {
      cv_id: cvId
    }, {
    ...
    if (batchResponse.data && batchResponse.data.success) {
      ...
      logger.info('Batch match calculation completed:', {
        ...
        successful_matches: result.matches_calculated,
        failed_matches: result.total_jobs - result.matches_calculated
      });

      // Log top 3 matches for monitoring
      if (result.results && result.results.length > 0) {
        const topMatches = result.results.slice(0, 3);
        logger.info('Top 3 matches calculated:', {
          cv_id: cvId,
      ...
      throw new Error(batchResponse.data?.error || 'Batch matching failed');
    }

  } catch (error) {
    logger.error('Error in calculateMatchScoresForCV:', {
      cv_id: cvId,
      error: error.message,
      stack: error.stack,
    ...
    // Fallback: if batch endpoint fails, log but don't throw to prevent blocking CV creation
    logger.warn('Batch matching failed, CV creation will continue without match calculation:', {
      cv_id: cvId,
      error: error.message
    });
...
async function extractCVContentAsync(cvId, cvFileUrl) {
  try {
    // Resolve full file path - handle both absolute and relative paths
    let fullFilePath;
    if (path.isAbsolute(cvFileUrl)) {
      fullFilePath = cvFileUrl;
    } else if (cvFileUrl.startsWith('/uploads/')) {
      // Remove leading slash and join with base directory
      fullFilePath = path.join(__dirname, '..', cvFileUrl.substring(1));
    } else {
      fullFilePath = path.join(__dirname, '..', cvFileUrl);
    }
    ...
    if (!fs.existsSync(fullFilePath)) {
      throw new Error(`CV file not found: ${fullFilePath}`);
    }

    // Create form data for AI extraction service with cv_id
    const formData = new FormData();
    formData.append('cv', fs.createReadStream(fullFilePath));
    formData.append('cv_id', cvId);
    
    // Call AI extraction service with cv_id
    const extractResponse = await axios.post(`${AI_EXTRACTION_SERVICE_URL}/extract-cv-with-id`, formData, {
      headers: {
        ...formData.getHeaders(),
      },
    ...
    if (extractResponse.data && extractResponse.data.success) {
      logger.info('CV extraction successful', {
        cv_id: cvId,
        database_saved: extractResponse.data.database_saved
      });

      // The AI service has already saved to PostgreSQL, so we just log success
      if (extractResponse.data.database_saved) {
        logger.info('CV content saved to PostgreSQL via AI service', {
          cv_id: cvId
        });
        return { success: true, message: 'CV extracted and saved to PostgreSQL' };
      } else {
        logger.warn('CV extracted but not saved to PostgreSQL', {
          cv_id: cvId,
          error: extractResponse.data.database_error
        });
      }

      // CV content has been processed and saved by AI service
      logger.info('CV content extraction completed via AI service', { 
        cv_id: cvId,
        database_saved: extractResponse.data.database_saved
      });

      // Auto-calculate match scores with available jobs (async, don't block)
      if (extractResponse.data.database_saved) {
        setTimeout(() => {
          calculateMatchScoresForCV(cvId).catch(matchError => {
            logger.warn('Failed to calculate match scores for CV:', {
              cv_id: cvId,
              error: matchError.message
            });
          });
        }, 3000); // Wait 3 seconds after extraction completes
      }
    }
  } catch (error) {
...
router.post('/', authenticateToken, requireRole(['CANDIDATE']), async (req, res) => {
  try {
    ...
    if (error) {
      console.error('❌ CV Creation Validation Error:', {
    ...
    extractCVContentAsync(cv.cv_id, cv.file_path).catch(error => {
  ...
  try {
    ...
    const matchQuery = `
      SELECT 
        ...
        j.company_id,
        vm.overall_similarity,
        vm.skills_similarity,
        vm.experience_similarity, 
        vm.education_similarity,
        vm.weighted_score,
        vm.last_calculated,
        ROUND(vm.weighted_score * 100, 2) as match_score
      FROM vector_matches vm
      JOIN jobs j ON vm.job_id = j.job_id  
      WHERE vm.cv_id = $1
      ORDER BY vm.weighted_score DESC
      LIMIT $2
    `;
    
    const matchResult = await cvModel.db.query(matchQuery, [cv_id, parseInt(limit)], 'get_cv_match_scores');
    
    if (matchResult.rows.length === 0) {
      return res.json({
        success: true,
        message: 'No match scores calculated yet',
        data: {
          cv_id,
    ...
    const avgScore = matches.reduce((sum, m) => sum + parseFloat(m.match_score), 0) / matches.length;

    const jobMatchScores = matches.map(match => ({
      job_id: match.job_id,
      job_title: match.job_title,
      company_id: match.company_id,
      match_score: parseFloat(match.match_score),
      detailed_scores: {
        overall_similarity: parseFloat(match.overall_similarity),
        skills_similarity: parseFloat(match.skills_similarity),
        experience_similarity: parseFloat(match.experience_similarity),
        education_similarity: parseFloat(match.education_similarity),
        weighted_score: parseFloat(match.weighted_score)
      },
      last_calculated: match.last_calculated
    }));
    ...
    res.json({
      ...
      data: {
        cv_id,
        best_match_score: parseFloat(bestMatch.match_score),
        best_match_job: bestMatch.job_title,
        has_job_matches: matches.length > 0,
        job_match_scores: jobMatchScores,
        total_matches: matches.length,
        average_score: Math.round(avgScore * 100) / 100,
        last_calculated: bestMatch.last_calculated,
        status: 'completed'
      }
    });
...
/**
 * @swagger
 * /api/v1/cvs/{cv_id}/calculate-match:
 *   post:
 *     summary: Trigger match calculation for a CV
 *     tags: [CVs]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Match calculation job started
 *       403:
 *         description: Access denied
 *       404:
 *         description: CV not found
 ...
 */
router.post('/:cv_id/calculate-match', authenticateToken, requireRole(['CANDIDATE']), async (req, res) => {
  try {
    const { cv_id } = req.params;
    const candidate_id = req.user.user_id;

    // Validate cv_id format
    if (!cvModel.isValidUUID(cv_id)) {
      return res.status(400).json({
        success: false,
        error: 'Invalid CV ID format'
    ...
    const cvQuery = `
      SELECT cv.cv_id 
      FROM candidate_cvs cv
      WHERE cv.cv_id = $1 AND cv.candidate_id = $2
    `;
    const cvResult = await cvModel.db.query(cvQuery, [cv_id, candidate_id], 'validate_cv_ownership');
    ...
    // Trigger match calculation job (async, don't block response)
    calculateMatchScoresForCV(cv_id).catch(error => {
      logger.error('Failed to calculate match scores:', {
        cv_id: cv_id,
        error: error.message
      });
    });
    ...
    res.json({
      success: true,
      message: 'Match calculation job started',
      data: {
        cv_id,
        status: 'calculating',
    ...
    logger.error('Failed to start match calculation:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      code: 'CALCULATE_MATCH_ERROR'
    });
  }
});
