...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialMode, onAuthSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'forgot-password') {
        await api.post('/api/v1/auth/forgot-password', { email: formData.email });
  ...
  const renderForgotPasswordForm = () => (
    <form onSubmit={handleSubmit} className="space-y-4">
       <h2 className="text-lg font-bold text-gray-900 mb-4">Forgot Password</h2>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex h-[550px]">
          ...
          <div className={`hidden lg:block lg:w-2/5 bg-gray-50 rounded-l-2xl p-6 pt-16 relative ${mode === 'forgot-password' ? 'lg:hidden' : ''}`}>
          ...
          {/* Right side - Form */}
          <div className={`w-full p-6 pt-6 lg:pt-8 rounded-2xl lg:rounded-l-none ${mode === 'forgot-password' ? 'lg:w-full' : 'lg:w-3/5'}`}>
                ...
                <form onSubmit={handleSubmit} className="space-y-2.5">
                  ...
                  {mode === 'login' && (
                    <div className="flex items-center justify-between">
                      ...
                      <button type="button" onClick={() => setMode('forgot-password')} className="text-xs text-[#007BFF] hover:underline">Forgot password?</button>
                    </div>
                  )}
